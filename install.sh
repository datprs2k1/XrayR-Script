...
install_XrayR() {
    ...
    if  [ $# == 0 ] ;then
        last_version=$(curl -Ls "https://api.github.com/repos/qtai2901/script_xrayr/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
	...
	echo ""
        wget -N --no-check-certificate -O /usr/local/XrayR/XrayR-linux.zip https://github.com/qtai2901/script_xrayr/releases/download/${last_version}/XrayR-linux-${arch}.zip
        if [[ $? -ne 0 ]]; then
	echo ""
        ...
        last_version=$1
        url="https://github.com/qtai2901/script_xrayr/releases/download/${last_version}/XrayR-linux-${arch}.zip"
	echo ""
    ...
    file="https://raw.githubusercontent.com/datprs2k1/XrayR-Script/main/XrayR.service"
